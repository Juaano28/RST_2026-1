...
// Agrega estas funciones al final de library_led_c.c

void rgb_led_wifi_app_started(void) {
    // Ejemplo: Amarillo (Rojo + Verde)
    apply_with_clamp(&my_rgb_led, my_rgb_led.led_red.duty_max, my_rgb_led.led_green.duty_max, 0);
}

void rgb_led_http_server_started(void) {
    // Ejemplo: Magenta (Rojo + Azul)
    apply_with_clamp(&my_rgb_led, my_rgb_led.led_red.duty_max, 0, my_rgb_led.led_blue.duty_max);
}

void rgb_led_wifi_connected(void) {
    // Ejemplo: Verde (Conexión exitosa)
    apply_with_clamp(&my_rgb_led, 0, my_rgb_led.led_green.duty_max, 0);
}